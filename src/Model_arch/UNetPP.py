...
class UNetPlusPlus():
    ...
    def BuildNetwork(self):
        """
        Creates the UNet++ Network for biomedical image segmentation.
    
        Returns:
            model: the compiled UNet++ model architecture.
        """
    
        input_img = Input(shape=self.__input_shape, name='InputLayer')
    
        conv00 = self.__InsertConvolutionBlock(input_img, '00', self.__filters[0])
        pool0 = MaxPooling2D(pool_size=2, strides=2, name='pool0')(conv00)
    
        conv10 = self.__InsertConvolutionBlock(pool0, '10', self.__filters[1])
        pool1 = MaxPooling2D(pool_size=2, strides=2, name='pool1')(conv10)
    
        up01 = Conv2DTranspose(self.__filters[0], 2, strides=2, padding='same', name='upsample01')(conv10)
        conv01 = concatenate([up01, conv00], name='concat01')
        conv01 = self.__InsertConvolutionBlock(conv01, '01', self.__filters[0])
    
        conv20 = self.__InsertConvolutionBlock(pool1, '20', self.__filters[2])
        pool2 = MaxPooling2D(pool_size=2, strides=2, name='pool2')(conv20)
    
        up11 = Conv2DTranspose(self.__filters[1], 2, strides=2, padding='same', name='upsample11')(conv20)
        conv11 = concatenate([up11, conv10], name='concat11')
        conv11 = self.__InsertSkipPathway(conv11, '11', self.__filters[1])
    
        up02 = Conv2DTranspose(self.__filters[0], 2, strides=2, padding='same', name='upsample02')(conv11)
        conv02 = concatenate([up02, conv00, conv01], name='concat02')
        conv02 = self.__InsertConvolutionBlock(conv02, '02', self.__filters[0])
    
        conv30 = self.__InsertConvolutionBlock(pool2, '30', self.__filters[3])
        pool3 = MaxPooling2D(pool_size=2, strides=2, name='pool3')(conv30)
    
        up21 = Conv2DTranspose(self.__filters[2], 2, strides=2, padding='same', name='upsample21')(conv30)
        conv21 = concatenate([up21, conv20], name='concat21')
        conv21 = self.__InsertConvolutionBlock(conv21, '21', self.__filters[2])
    
        up12 = Conv2DTranspose(self.__filters[1], 2, strides=2, padding='same', name='upsample12')(conv21)
        conv12 = concatenate([up12, conv10, conv11], name='concat12')
        conv12 = self.__InsertSkipPathway(conv12, '12', self.__filters[1])
    
        up03 = Conv2DTranspose(self.__filters[0], 2, strides=2, padding='same', name='upsample03')(conv12)
        conv03 = concatenate([up03, conv00, conv01, conv02], name='concat03')
        conv03 = self.__InsertConvolutionBlock(conv03, '03', self.__filters[0])
    
        conv40 = self.__InsertConvolutionBlock(pool3, '40', self.__filters[4])
    
        up31 = Conv2DTranspose(self.__filters[3], 2, strides=2, padding='same', name='upsample31')(conv40)
        conv31 = concatenate([up31, conv30], name='concat31')
        conv31 = self.__InsertSkipPathway(conv31, '31', self.__filters[3])
    
        up22 = Conv2DTranspose(self.__filters[2], 2, strides=2, padding='same', name='upsample22')(conv31)
        conv22 = concatenate([up22, conv20, conv21], name='concat22')
        conv22 = self.__InsertSkipPathway(conv22, '22', self.__filters[2])
    
        up13 = Conv2DTranspose(self.__filters[1], 2, strides=2, padding='same', name='upsample13')(conv22)
        conv13 = concatenate([up13, conv10, conv11, conv12], name='concat13')
        conv13 = self.__InsertSkipPathway(conv13, '13', self.__filters[1])
    
        up04 = Conv2DTranspose(self.__filters[0], 2, strides=2, padding='same', name='upsample04')(conv13)
        conv04 = concatenate([up04, conv00, conv01, conv02, conv03], name='concat04')
        conv04 = self.__InsertConvolutionBlock(conv04, '04', self.__filters[0])
    
        # Output layers with sigmoid
        nested_op_1 = Conv2D(self.__nb_classes, 1, activation=tf.nn.sigmoid, padding='same', name='op1')(conv01)
        nested_op_2 = Conv2D(self.__nb_classes, 1, activation=tf.nn.sigmoid, padding='same', name='op2')(conv02)
        nested_op_3 = Conv2D(self.__nb_classes, 1, activation=tf.nn.sigmoid, padding='same', name='op3')(conv03)
        nested_op_4 = Conv2D(self.__nb_classes, 1, activation=tf.nn.sigmoid, padding='same', name='op4')(conv04)
    
        # Final output
        if self.__deep_supervision:
            output = [nested_op_1, nested_op_2, nested_op_3, nested_op_4]
        else:
            output = nested_op_4  # ✅ return tensor directly, not a list
